      ...
      <form onSubmit={handleSubmit}>
        <FieldGroup>
          <div className="flex flex-col items-center gap-1 text-center mb-4">
            <h1 className="text-3xl font-display font-black tracking-tighter text-white uppercase">{t('auth.login_title')}</h1>
          ...
          <Field>
            <div className="flex items-center justify-between ml-1">
              <FieldLabel htmlFor="password" className="font-bold uppercase text-[10px] tracking-widest">{t('auth.password')}</FieldLabel>
              <Link
                href="/forgot-password"
                className="text-xs text-blue-500 hover:underline underline-offset-4 font-bold"
              >
          ...
          <Field>
            <Button type="submit" className="w-full bg-blue-600 hover:bg-blue-500 text-white font-black uppercase tracking-tight h-11 shadow-lg shadow-blue-600/20" disabled={loading}>
              {loading ? t('auth.login_loading') : t('auth.login_btn')}
            </Button>
          </Field>

          {error && (
            <div className="p-3 rounded-lg bg-red-500/10 border border-red-500/20 text-red-500 text-xs text-center font-bold">
